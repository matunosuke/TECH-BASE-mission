...
<body>
    ...
    好きな食べ物を一つ書いてください！<br><br>
     ...
     if(isset($_POST['edit'])){
              ...
              $password = $_POST['pass_3'];
              //もし編集番号が空でなく、パスワードがあっていたら以下を実行
              if($edi_num != null && $password == "pass"){
                if(file_exists($filename)){//もしファイルが存在するなら
                  ...
                  foreach($lines as $line){//lines内の配列をfileに代入
                     ...
                     if($e_line[0] == $edi_num && $e_line[4] == $password){
                         $edi_name = $e_line[1];
            ...
            if($after_edi_num != null){
                if(file_exists($filename)){//もしファイルが存在するなら
                  ...
                  //編集後のファイル内容表示
                   ...
                   if(file_exists($filename)){//もしファイルが存在するなら
                    ...
                    echo "--------------------<br>";
            }elseif($after_edi_num == null && $password == 'pass'){
             ...
             //追記する場合
                ...
                if($name != null && $comment != null){//もしnameとcommentが空じゃないとき以下の動作
                    ...
                    $str = $num ."<>" . $name ."<>" . $comment . "<>" .$date . "<>" .$password. "<>";
